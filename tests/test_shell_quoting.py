...
def test_parameter_name_collision():
    ...
    try:
        result = executor.execute_action(action, {"A": "First Value", "AB": "Second Value"})
...
def test_parameter_order_dependency():
    ...
    action = Action(
        ...
        parameters=[
            ActionParameter("PREFIX_SUFFIX", ParameterType.INSECURE_STRING, "Full parameter"),
            ActionParameter("PREFIX", ParameterType.INSECURE_STRING, "Prefix parameter"),
        ],
    )
    
    try:
        result = executor.execute_action(action, {"PREFIX": "Hello", "PREFIX_SUFFIX": "Hello World"})
        
        # Parameter order should be handled correctly
        assert "Hello Hello World" == result['stdout'], f"Expected 'Hello Hello World' in output, got: {result['stdout']}"
...
def test_special_characters_in_values():
    ...
    action = Action(
        ...
        parameters=[
            ActionParameter("MESSAGE", ParameterType.INSECURE_STRING, "Message with special chars"),
        ],
    )
    ...
    try:
        for test_name, message in test_cases:
            ...
            assert message == result['stdout'], f"Expected '{message}' in output, got: {result['stdout']} for test {test_name}"
