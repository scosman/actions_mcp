    ...
    for action in config.actions:
        ...
        tool = Tool(
            ...
            inputSchema={
                ...
                "required": [param["name"] for param in parameters if "default" not in param]
            }
        )
        tools.append(tool)
...
async def serve(ActionsMCP_config: ActionsMCPConfig) -> None:
    ...
    async def call_tool(name: str, arguments: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ExecutionError(f"ActionsMCP Error: Unexpected error executing action '{name}': {str(e)}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="ActionsMCP - MCP server for project-specific development tools")
    parser.add_argument(
        "config_path",
        nargs="?",
    ...
    if missing_vars:
        print(f"ActionsMCP Error: Required environment variables not set: {', '.join(missing_vars)}")
