...
class ActionParameter:
    ...
    def __init__(self, name: str, param_type: str, description: Optional[str] = None, 
...
class Action:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Action':
        ...
        if not description:
            raise ConfigError("ActionsMCP Error: 'description' is required for each action")
        if not command:
        ...
        if "parameters" in data:
            for param_data in data["parameters"]:
                ...
                if not param_name:
                    raise ConfigError(f"ActionsMCP Error: 'name' is required for each parameter in action '{name}'")
                if not param_type:
                    raise ConfigError(f"ActionsMCP Error: 'type' is required for parameter '{param_name}' in action '{name}'")
                
                if param_type not in [ParameterType.PROJECT_FILE_PATH, ParameterType.REQUIRED_ENV_VAR, 
                                      ParameterType.OPTIONAL_ENV_VAR, ParameterType.INSECURE_STRING]:
                    raise ConfigError(f"ActionsMCP Error: Invalid parameter type '{param_type}' for parameter '{param_name}' in action '{name}'. "
                                      f"Valid types are: project_file_path, required_env_var, optional_env_var, insecure_string")
                
                parameters.append(ActionParameter(param_name, param_type, param_description, param_default))
...
class ActionsMCPConfig:
    ...
    def from_yaml(cls, yaml_path: str) -> 'ActionsMCPConfig':
        ...
        if not os.path.exists(yaml_path):
            raise ConfigError(f"ActionsMCP Error: Configuration file '{yaml_path}' not found")
        
        try:
            with open(yaml_path, 'r') as f:
                data = yaml.safe_load(f)
        except yaml.YAMLError as e:
            raise ConfigError(f"ActionsMCP Error: Failed to parse YAML file '{yaml_path}': {str(e)}")
        except Exception as e:
            raise ConfigError(f"ActionsMCP Error: Failed to read configuration file '{yaml_path}': {str(e)}")
        
        if not isinstance(data, dict):
            raise ConfigError("ActionsMCP Error: Configuration file must contain a YAML object")
        
        actions_data = data.get("actions")
        if not actions_data:
            raise ConfigError("ActionsMCP Error: 'actions' array is required in configuration file")
        ...
        for i, action_data in enumerate(actions_data):
            if not isinstance(action_data, dict):
                raise ConfigError(f"ActionsMCP Error: Each action must be an object (action[{i}])")
            try:
                ...
                raise ConfigError(f"ActionsMCP Error: Failed to parse action[{i}]: {str(e)}")
